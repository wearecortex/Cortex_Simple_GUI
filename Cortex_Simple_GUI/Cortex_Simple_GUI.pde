...
final String playbackData_fname = "test.txt"; //only used if loading input data from a file
//final String playbackData_fname = "test.txt"; //only used if loading input data from a file
...
void setup() {
  logo =  loadImage("cortexLogo2.png");
  //open window
...
//  gui = new gui_Manager(this, win_x, win_y, nchan, displayTime_sec,vertScale_uV,fs_Hz,filterText,signalDetectName);
  ...
  //prepare the source of the input databada
  ...
  // initialize the minim and audioOut objects...specific to Cortex_Simple_GUI
...
void draw() {
  ...
  if (isRunning) {
    ...
    if (pointCounter >= nPointsPerUpdate) {
      ...
      //try to detect the desired signals, do it in frequency space...for Cortex_Simple_GUI
